...
def run_one_branch(parent_dir: Path, branch_dir: Path, output_root: Path) -> None:
    ...
    if DO_PLOTS:
        if shaft_clusters:
            ...
            save_current_fig(plots_out / "shaft_cluster_bins.png")

            plot_markers_along_branch_with_ids(
                res, shaft_clusters, title="Markers along branch (scaled) — shaft clusters"
            )
            save_current_fig(plots_out / "shaft_markers_along_branch_with_ids.png")

        if spine_clusters:
            plot_cluster_bins(spine_clusters, title="Spine clusters (TP presence)")
            save_current_fig(plots_out / "spine_cluster_bins.png")

            plot_markers_along_branch_with_ids(
                res, spine_clusters, title="Markers along branch (scaled) — spine clusters"
            )
            save_current_fig(plots_out / "spine_markers_along_branch_with_ids.png")
...
def main():
    ...
    output_root = parent_dir / "AutoalignmentOutputs"

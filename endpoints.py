...
def get_branch_run_endpoints_from_trace(
    ...
    """
    Gets the first and last row from each consecutive run of branch_id.

    If branch 1 appears, then later branch 2, then later branch 1 again,
    this treats those as separate branch 1 runs.

    For your current per-branch files, it returns the first run matching branch_id.
    """
    ...
    df["_run_id"] = (df[branch_col] != df[branch_col].shift()).cumsum()

    branch_runs = []

    for _, run in df.groupby("_run_id", sort=False):
        if int(run[branch_col].iloc[0]) == int(branch_id):
            branch_runs.append(run)

    if not branch_runs:
        raise ValueError(f"No consecutive trace run found for branch {branch_id}")

    # use the first consecutive run for this branch_id
    branch_run = branch_runs[0]

    start = branch_run.iloc[0].loc[list(coord_cols)].to_numpy(dtype=float)
    end = branch_run.iloc[-1].loc[list(coord_cols)].to_numpy(dtype=float)

    # match graph scaling: x/y scaled, z not scaled
    start[0] *= 0.25
    start[1] *= 0.25

...
images_root = Path(r'Z:\Joe\AnalysisCode_forSharing\ExampleData\FullyAnalyzed\Analysis_withAmyCode_cell1\SNTTrace')
puncta_root = Path(r'Z:\Joe\AnalysisCode_forSharing\ExampleData\FullyAnalyzed\Analysis_withAmyCode_cell1\AlignmentOutputsSOM056')
...
csv_img_pat =re.compile(r"_?Image(\d+)_branch\d+", re.IGNORECASE)
...
def run_one_branch(
    ...
    *,
    ...
    #this said t = threshold_otsu(boutons) + otsu_offset. Joe changed to t = 15 for constant threshold (specific number varies between cells based on background measured by Joe)
    t = 15
...
# puncta_root = Path('/Volumes/nedividata/Joe/2p_data/SOM/ThirdRound/SOM026_DOB081520_RV/Analysis/Analysis_withAmyCode/AutoalignmentOutputs1')

# CHANNEL_INDEX = 0
# BOUTON_CHANNEL_INDEX = 2
...
# SHOW_NAPARI = False
# NAPARI_ONLY_IMAGE = None   # e.g. 0
# NAPARI_ONLY_BRANCH = None  # e.g. "1"
...
# def to_image_zyx(points_zyx_scaled: np.ndarray, z_scale: int) -> np.ndarray:
#     p = points_zyx_scaled.astype(np.float32, copy=True)
#     p[:, 0] /= z_scale
#     return p


# def show_napari_debug_view(
#     *,
#     img_zycx: np.ndarray,
#     other_channel: np.ndarray,
#     boutons: np.ndarray,
#     pts: np.ndarray,
#     pts_snapped_all: np.ndarray,
#     disk_mask: np.ndarray,
#     boutons_mask: np.ndarray,
#     coloc_mask: np.ndarray,
#     coords3d_global: np.ndarray | None,
#     z_scale: int,
#     disk_r: int,
...
#     csv_path: Path,
# ):
#     pts_img = to_image_zyx(pts, z_scale)
#     pts_snapped_img = to_image_zyx(pts_snapped_all, z_scale)

#     peaks_img = None
#     if coords3d_global is not None and len(coords3d_global) > 0:
#         peaks_img = to_image_zyx(coords3d_global, z_scale)

#     viewer = napari.Viewer(ndisplay=3)
#     viewer.title = f"Image{image_num} | {csv_path.name}"

#     viewer.add_image(other_channel, name="other_channel_ch0", colormap="gray")
#     viewer.add_image(img_zycx[:, 1, :, :], name="other_channel_ch1", colormap="gray")
#     viewer.add_image(boutons, name="boutons_ch2", colormap="magenta", opacity=0.6)

#     viewer.add_labels(
#         boutons_mask.astype(np.uint8),
#         name=f"boutons_mask_t{threshold_value:.2f}"
#     )
#     viewer.add_labels(
#         disk_mask.astype(np.uint8),
#         name=f"snapped_disks_r{disk_r}"
#     )
#     viewer.add_labels(
#         coloc_mask.astype(np.uint8),
#         name="colocalization"
#     )

#     viewer.add_points(
#         pts_img,
#         name=f"pts_original_n{len(pts_img)}",
#         size=2,
#         face_color="red",
#         opacity=0.8,
#     )
...
# def should_show_napari(img_num: int, csv_path: Path) -> bool:
#     if not SHOW_NAPARI:
#         return False

#     if NAPARI_ONLY_IMAGE is not None and img_num != NAPARI_ONLY_IMAGE:
#         return False

#     if NAPARI_ONLY_BRANCH is not None:
#         m = re.search(r"branch(\d+)", csv_path.name, re.IGNORECASE)
#         branch_num = m.group(1) if m else None
#         if branch_num != str(NAPARI_ONLY_BRANCH):
#             return False
...
#         peaks = coords3d_global[mask_peaks]
#         peaks_xy = peaks[:, 1:3].astype(np.float32)
#         query_xy = pts[idx_p][:, 1:3].astype(np.float32)
...
#     # use your current constant threshold
#     t = 10
#     boutons_mask = boutons > t
#     coloc_mask = boutons_mask & disk_mask.astype(bool)
...
#     if should_show_napari(img_num, csv_path):
#         show_napari_debug_view(
#             img_zycx=img_zycx,
#             other_channel=other_channel,
#             boutons=boutons,
#             pts=pts,
#             pts_snapped_all=pts_snapped_all,
#             disk_mask=disk_mask,
#             boutons_mask=boutons_mask,
#             coloc_mask=coloc_mask,
#             coords3d_global=coords3d_global,
#             z_scale=z_scale,
#             disk_r=disk_r,
#             threshold_value=t,
#             image_num=img_num,

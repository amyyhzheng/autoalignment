...
def _estimate_coordinate(tp: int, avg_dist: float, result: ComputationResult) -> Coord:
    ...
    #Joe added the lines below for debugging purposes
    print(f"DEBUG: tp={tp}, avg_dist={avg_dist}")
    print(f"DEBUG: cum range = [{cum[0]}, {cum[-1]}]")
    print(f"DEBUG: cum length = {len(cum)}")
    for seg_id in range(len(cum)-1):
        if avg_dist <= cum[seg_id+1]:
            ...
            for i in range(len(result.normalized_branch[tp])):
                ...
                #Joe modified the line below for debugging purposes, prints the problematic value. distance out of bounds is usually related to an error with SNT tracing and fixed by retracaing the relevant branch
    raise ValueError(f"average distance {avg_dist} out of bounds [0, {cum[-1] if cum else 'empty'}]")
...
#     next_id = 1
#     if shaft_clusters:
#         part, next_id = clusters_to_csv_rows(shaft_clusters, "Shaft_SyntdNotScored", "Empty_shaft", next_id, False, settings, result)
#         rows.extend(part)
#     if spine_clusters:
#         part, next_id = clusters_to_csv_rows(spine_clusters, "Spine_SynTdNotScored", "Empty_spine", next_id, True, settings, result)
#         rows.extend(part)
...
#     for tp, nap_rows in by_tp.items():
#         #changed to match puncta scoring format
#         tp_int = int(tp) 

#         path = os.path.join(
#             auto_dir,
#             f"Image{tp_int - 1}_branch{settings.branch_id}.csv",
#         )
#         with open(path, 'w', newline='') as f:
#             wr = csv.writer(f)
#             wr.writerow(header)
...
def clusters_to_csv_rows(
    ...
    for cluster in cluster_list:
        ...
        for tp, idx, pos in cluster:
            if idx == "NA":
                # Missing observation: estimate from branch geometry
                mtype = empty_type
                x, y, z = _estimate_coordinate(tp, avg_dist, result)
                x += dx
                y += dy
            else:
                # Real observation: recover ORIGINAL raw marker coordinate exactly
                mtype = marker_type
                x, y, z = result.raw_marker_coords_only[tp][idx]

            out.append([
                f"Image{tp+1}",
                next_id,   # keep original integer markerID style
                mtype,
                x,
                y,
...
def export_all(settings: Settings, result: ComputationResult,
    ...
    if shaft_clusters:
        part, next_id = clusters_to_csv_rows(
            shaft_clusters, "Shaft_SyntdNotScored", "Empty_shaft",
            next_id, False, settings, result
        )
        rows.extend(part)
    ...
    out_csv = os.path.join(
        auto_dir,
        f"{settings.animal_id}_b{settings.branch_id}_alignmentMapping.csv"
    )
    ...
    # -------- DUPLICATE CHECK --------
    print("\nChecking duplicates within each timepoint...")

...
def configure_viewer(viewer, image, viewer_index):
    ...
    # Check the number of channels in the image
       ...
       # z_spacing = 0.8
        ...
        viewer.layers['Ch1: RFP'].contrast_limits = (3, 15)
       ...
       # this was originally for Bettina MAP with z_spacing = 0.8 but Joe modified to be able to load a 4-channel image including normalized ch4 gephyrin 

        ch1, ch2, ch3, ch4 = image[:, 0, :, :], image[:, 1, :, :], image[:, 2, :, :], image[:, 3, :, :]
        viewer.add_image(ch1, name='Ch1: Gephyrin', blending='additive', colormap='green', scale = [4, 1, 1])
        viewer.layers['Ch1: Gephyrin'].contrast_limits = (2, 17)
        viewer.add_image(ch2, name='Ch2: Cell Fill', blending='additive', colormap='red', scale = [4, 1, 1])
        viewer.layers['Ch2: Cell Fill'].contrast_limits = (0, 40)
        viewer.add_image(ch3, name='Ch3: Syntd', blending='additive', colormap='cyan', scale = [4, 1, 1]) 
        viewer.layers['Ch3: Syntd'].contrast_limits = (10, 80)
        viewer.add_image(ch4, name='Ch4: NormGeph', blending='additive', colormap='white', scale = [4, 1, 1])
        #Amy's original code is below
        #viewer.add_image(ch1, name='Ch1: RFP', blending='additive', colormap='cyan')
        #viewer.add_image(ch2, name='Ch2: Gephyrin', blending='additive', colormap='green')
        #viewer.add_image(ch3, name='Ch3: Cell Fill', blending='additive', colormap='white')
        #viewer.add_image(ch4, name='Ch4: Bassoon', blending='additive', colormap='red')
...
def normalized_puncta(ch1, ch2, ch3, viewer):
    ...
    geph_mask = ch1_copy >= 3 #requires the gephyrin brightness to be at least 3 in any areas that will later be considered puncta
    geph_dend_mask = ch2 >= 5 #Joe addition 7/28/25 enabling an additional masking step where the cell fill bgithness must be at least 5 in any areas that will later be considered puncta

    # Apply the mask to normch4
    normch4_dendrites = np.where(dendrite_mask, normch4, 0)
    normch4_gephyrin = np.where(geph_mask,normch4, 0 ) #pixels that will be considered as part of puncta must be above brightness threshold defined above for teal-gephyrin channel (ch1)
    normch4_gephyrin = np.where(geph_dend_mask, normch4_gephyrin, 0)  #Joe additionl 7/28/25, additional masking step so that pixels that will later be considered puncta must be above brightness threshold in the cell fill channel (ch2)
    ...
    print(f"Standard deviation: {std_intensity}")
       ...
       #Joe commented out the entire sectino below 7/28/25 to make images load faster 

...
def getUserInput():
    ...
    marker_filename = '/Volumes/nedividata/Joe/2p_data/SOM/ThirdRound/SOM023_DOB073020LV/Analysis/Images_blinded_names/AutoScore/PunctaScoring/b2/SynapseMarkers/beforeAlignment/CombinedResults.csv'
    fiducial_fileName = '/Volumes/nedividata/Joe/2p_data/SOM/ThirdRound/SOM023_DOB073020LV/Analysis/Images_blinded_names/AutoScore/PunctaScoring/b2/Fiducials/CombinedResults.csv'

    xyz_fileNames['Timepoint 1'] = '/Volumes/nedividata/Joe/2p_data/SOM/ThirdRound/SOM023_DOB073020LV/Analysis/Images_blinded_names/AutoScore/SNT/Image1/SOM023_Image1_FullTrace_withbrancheslabeled_xyzCoordinates.csv'
    xyz_fileNames['Timepoint 2'] = '/Volumes/nedividata/Joe/2p_data/SOM/ThirdRound/SOM023_DOB073020LV/Analysis/Images_blinded_names/AutoScore/SNT/Image2/SOM023_Image2_FullTrace_withbrancheslabeled_xyzCoordinates.csv'
    xyz_fileNames['Timepoint 3'] = '/Volumes/nedividata/Joe/2p_data/SOM/ThirdRound/SOM023_DOB073020LV/Analysis/Images_blinded_names/AutoScore/SNT/Image3/SOM023_Image3_FullTrace_withbrancheslabeled_xyzCoordinates.csv'
    xyz_fileNames['Timepoint 4'] = '/Volumes/nedividata/Joe/2p_data/SOM/ThirdRound/SOM023_DOB073020LV/Analysis/Images_blinded_names/AutoScore/SNT/Image4/SOM023_Image4_FullTrace_withbrancheslabeled_xyzCoordinates.csv'
    xyz_fileNames['Timepoint 5'] = '/Volumes/nedividata/Joe/2p_data/SOM/ThirdRound/SOM023_DOB073020LV/Analysis/Images_blinded_names/AutoScore/SNT/Image5/SOM023_Image5_FullTrace_withbrancheslabeled_xyzCoordinates.csv'
    xyz_fileNames['Timepoint 6'] = '/Volumes/nedividata/Joe/2p_data/SOM/ThirdRound/SOM023_DOB073020LV/Analysis/Images_blinded_names/AutoScore/SNT/Image6/SOM023_Image6_FullTrace_withbrancheslabeled_xyzCoordinates.csv'
...
for segments_in_timepoint in segment_distance_allTimepoints:
    ...
    for segment_distances in maxDist_and_segmentDist:
        if segment_distances[1] ==0:
            print("WOULD HAVE HAD 0 DIVISION ERROR - ASSUMED DISTANCE IS 0.01")
...
def calculateClustersGMM(distance_data, filename):
    '''
    Use Gaussian Mixture Models to cluster the synapses
    '''
    # Prepare data
    all_distances = []
    timepoint_indices = []
    ...
    for tp_idx, timepoint in enumerate(distance_data):
        for point_idx, distance in enumerate(timepoint):
            all_distances.append([distance])
            timepoint_indices.append(tp_idx)
    ...
    X = scaler.fit_transform(all_distances)
    
    # Find optimal number of components using BIC
    n_components_range = range(1, min(len(all_distances), 10))  # Max 10 clusters
    bic = []
    ...
    print(f"Optimal number of clusters for {filename}: {optimal_n_components}")
    ...
    clusters = [[] for _ in range(optimal_n_components)]
    
    for i, (label, tp_idx, point_idx, distance) in enumerate(zip(labels, timepoint_indices, point_indices, all_distances)):
        clusters[label].append((tp_idx, point_idx, distance[0]))
    
    # Sort clusters by mean distance
    cluster_means = []
    for cluster in clusters:
        mean_dist = np.mean([x[2] for x in cluster])
        cluster_means.append(mean_dist)
    ...
    for cluster in sorted_clusters:
        cluster_points = []
        for timepoint in range(len(distance_data)):
            points_in_tp = [p for p in cluster if p[0] == timepoint]
            if points_in_tp:
                for point in points_in_tp:
                    cluster_points.append((point[0], point[1], point[2]))
            else:
                cluster_points.append((timepoint, 'NA', 'NA'))
        formatted_clusters.append(sorted(cluster_points))
    
    # Save cluster info
    save_cluster_info(formatted_clusters, gmm, scaler, filename)
    
    return formatted_clusters

def save_cluster_info(clusters, gmm, scaler, filename):
    '''
    Save detailed information about the clusters
    '''
    newpath = os.path.join(export_csv_directory, 'clusterInfo')
    if not os.path.exists(newpath):
        os.makedirs(newpath)
    
    with open(os.path.join(newpath, f'{animal_ID}_b{branch_ID}_{filename}_cluster_info.txt'), 'w') as f:
        f.write(f"Number of clusters: {len(clusters)}\n\n")
        
        for i, cluster in enumerate(clusters):
            real_points = [(tp, idx, dist) for tp, idx, dist in cluster if idx != 'NA']
            if real_points:
                distances = [dist for _, _, dist in real_points]
                mean = np.mean(distances)
                std = np.std(distances)
                
                f.write(f"Cluster {i+1}:\n")
                f.write(f"  Mean distance: {mean:.2f}\n")
                f.write(f"  Standard deviation: {std:.2f}\n")
                f.write(f"  Points: {len(real_points)}\n")
                f.write(f"  Timepoints present: {sorted(set(tp for tp, _, _ in real_points))}\n\n")

# Replace the existing clustering calls
shaft_distances, spine_distances = separateShaftfromSpine()

if any(shaft_distances):
    shaft_grouping_list = calculateClustersGMM(shaft_distances, "inhibitoryshaft")

if any(spine_distances):
    spine_grouping_list = calculateClustersGMM(spine_distances, "inhibitoryspine")
...
def testResults(actual_csv_path, predicted_csv_list):
    ...
    for image in range(1, 7):
        ...
        # Collect actual points data (skipping NaN values and landmarks)
        valid_rows = df_actual[
            (~pd.isna(df_actual[final_col])) & 
            (df_actual[final_col].str.lower() != 'landmark')
        ]
        ...
        # Get predicted markers (filtering out landmarks)
        pred_image = predicted_df[
            (predicted_df['image'] == image_name) & 
            (predicted_df['markerType'].str.lower() != 'landmark')
        ]
    ...
    # Calculate metrics excluding nothing/landmark/nude labels
    excluded_labels = {'nothing', 'landmark', 'nudespine'}
    
    # Filter incorrect predictions
    filtered_incorrect = [info for info in incorrect_predictions_info 
                         if info['type'].lower() not in excluded_labels]
    
    # Filter missed actuals
    filtered_missed = [info for info in missed_actuals_info 
                      if info['type'].lower() not in excluded_labels]
    
    # Filter correct predictions - need to get their types from the predicted data
    filtered_correct = []
    filtered_correct_labels = []
    for i, point in enumerate(correct_points):
        # Parse the label to get predicted group number (format is "P{pred}/A{actual}")
        pred_group = int(correct_labels[i].split('/')[0][1:])  # Remove 'P' and get number
        
        # Find the type for this prediction
        pred_type = predicted_df[predicted_df['markerID'] == pred_group]['markerType'].iloc[0]
        
        if pred_type.lower() not in excluded_labels:
            filtered_correct.append(point)
            filtered_correct_labels.append(correct_labels[i])
    
    filtered_correct_predictions = len(filtered_correct)
    
    # Calculate filtered metrics
    filtered_total_actual = filtered_correct_predictions + len(filtered_missed)
    filtered_total_predicted = filtered_correct_predictions + len(filtered_incorrect)
    
    filtered_precision = filtered_correct_predictions / filtered_total_predicted if filtered_total_predicted > 0 else 0
    filtered_recall = filtered_correct_predictions / filtered_total_actual if filtered_total_actual > 0 else 0
    ...
    print(f"\nAlignment Results (Including all labels):")
    ...
    print(f"\nAlignment Results (Excluding nothing/landmark/nude labels):")
    print(f"Total actual markers: {filtered_total_actual}")
    print(f"Total predicted markers: {filtered_total_predicted}")
    print(f"Correctly predicted markers: {filtered_correct_predictions}")
    print(f"Precision: {filtered_precision:.2f}")
    print(f"Recall: {filtered_recall:.2f}")
    print(f"F1 Score: {filtered_f1:.2f}")
...
metrics = testResults('/Volumes/nedividata/Joe/2p_data/SOM/ThirdRound/SOM023_DOB073020LV/Analysis/Images_blinded_names/AutoScore/PunctaScoring/b2/SynapseMarkers/Aligned_afterManualCheck/CombinedResults.csv', testing_csv)
napari.run()
print(metrics)

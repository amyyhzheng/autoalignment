...
BASE_BRANCH_DIR = Path(
    "/Volumes/nedividata/Joe/2p_data/SOM/ThirdRound/SOM055_DOB051322_TT/Analysis_withAmyCode_cell4/AlignmentAndChecking/AfterManualEdits"
).expanduser().resolve()
...
def collect_rows_by_id(export_dir: Path, branch_id: str) -> Dict[str, List[dict]]:
    ...
    for path in files:
        ...
        for row in rows:
            ...
            print(f"  label={label} | type={row.get('type')} | from={path.name}")
...
def exported_csv_path(export_dir: Path, branch_id: str, tp_zero_based: int) -> Path:
    ...
    matches = list(
        export_dir.rglob(
            f"Image{tp_zero_based}_branch{branch_id}_snapped_bouton_overlap.csv"
        )
    )

    if not matches:
        raise FileNotFoundError(
            f"Could not find Image{tp_zero_based}_branch{branch_id}_snapped_bouton_overlap.csv under {export_dir}"
        )
...
def run_one_branch_one_tp(
    ...
    export_dir = marker_branch_dir

    print("\n" + "=" * 80)
    print(f"BRANCH {branch_id} | TARGET TP {target_tp_zero_based}")
    print(f"Marker dir: {marker_branch_dir}")
    print("=" * 80)
...
def main():
    for branch_id in BRANCH_IDS:
        ...
        if not marker_branch_dir.exists():
            print(f"[skip] Missing branch dir: {marker_branch_dir}")
            continue

        for target_tp_zero_based in TARGET_TPS_ZERO_BASED:
            try:
                run_one_branch_one_tp(
                    marker_branch_dir=marker_branch_dir,
                    branch_id=branch_id,
                    target_tp_zero_based=target_tp_zero_based,
                )
            except Exception as e:
                print(
                    f"[ERROR] Branch {branch_id}, TP {target_tp_zero_based} failed: {e}"
                )
